...
4. Run CREATE USER 'username'@'host' ('username'@'%' for all IPs) IDENTIFIED BY 'password'; where host is local IP
5. Run GRANT ALL PRIVILEGES ON database_name.* TO 'username'@'host' ('username'@'%' for all IPs);
...
2. added (at the end of the file) an extra line to pg_hba.conf to allow conections from my local IP address:
    ...
    (host all all 0.0.0.0/0 scram-sha-256     for all IPs)
4. added an inbound rule on azure for the vm for port 5432 (postgres standard)
5. troublshot vm firewall to allow inbound connections for postgress:
