...
class Graph:
    ...
    def query(self, query: str):
        records, summary, keys = self._driver.execute_query(query)
    ...
    def shortest_path_traversal(
        ...
        query_result = self.query(query)[
            0
        ]  # It's only 1 result anyway, just delisting it
    ...
    def add_pandas_table_node(self, pandas_dataframe, name, primary_key, labels):
        ...
        with self._driver.session() as session:
            session.execute_write(
                _create_node, pandas_dataframe, name, primary_key, labels
            )
    ...
    def add_relationship(self, name_node_one, name_node_two, relation_name):
        ...
        with self._driver.session() as session:
            session.execute_write(
                _add_relation, name_node_one, name_node_two, relation_name
            )
    ...
    def add_list_key_relationships(self, table_relationships: list, relation_name: str):
        for relationship in table_relationships:
            ...
            self.add_relationship(
                name_node_one=node_one, name_node_two=node_two, relation_name=key
            )
    ...
    def delete_relationship(self, name_node_one, name_node_two, relation_name):
        ...
        with self._driver.session() as session:
            session.execute_write(
                _delete_relation, name_node_one, name_node_two, relation_name
            )

    ## Bidirectional join function instead of single direction
    def add_bidirectional_relationship(
        self, name_node_one, name_node_two, name_relationship
    ):
    ...
    def add_join_table_relationship(
        self, name_node_one, name_node_two, name_join_table
    ):
    ...
    def add_triple(self, name1, name2, relation, description1, description2):
        with self._driver.session() as session:
            session.execute_write(
                self._add_triple, name1, name2, relation, description1, description2
            )
...
if __name__ == "__main__":
    ...
    g = Graph(
        os.getenv("GRAPH_URI"), os.getenv("GRAPH_USER"), os.getenv("GRAPH_PASS")
    )  # replace with your user, pass

    g = Graph(
        "bolt://localhost:7687", "neo4j", "eternal-pyramid-corner-jester-bread-6973"
    )
