Note: It seems like the embeddings will largely depend on what we want to use as the LLM (mainly for the efficiency of not having to compute it again). However, we can decouple them, and make our embeddings entirely separate from what is inputted to the LLM. This won't be too performance intensive, as we only ever pass in the relevant subset to the LLM (this always happens after reduction by search via attn mechanism or semantic search)
