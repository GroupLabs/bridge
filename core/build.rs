...
fn main() {
    ...
    // Try local faiss/ first, then system /usr/local/include
    let faiss_include = if std::path::Path::new("faiss/c_api/Index_c.h").exists() {
        "faiss/c_api"
    } else {
    ...
    let faiss_bindings = bindgen::Builder::default()
        .header(format!("{}/Index_c.h", faiss_include))
        .header(format!("{}/IndexFlat_c.h", faiss_include))
        .header(format!("{}/clone_index_c.h", faiss_include))
        .header(format!("{}/index_factory_c.h", faiss_include))
        .header(format!("{}/index_io_c.h", faiss_include))
        .header(format!("{}/AutoTune_c.h", faiss_include))
        .header(format!("{}/error_c.h", faiss_include))
        .header(format!("{}/IndexIVF_c.h", faiss_include))
        .clang_arg(format!("-I{}", faiss_include))
